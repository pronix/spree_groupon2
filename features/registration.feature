...
@javascript
  Сценарий: Регистрация без заполнения профайла
    ...
    И я перехожу по ссылке "Вход для клиентов"
    И я перехожу по ссылке "Создать новую учетную запись"
    ...
    И я увижу "Мобильный телефон: не может быть пустым"

    И я в поле "user_email" запишу "user@ya.ru"
    И я выберу "Украина" из "country"
    И спать 1 секунды
    И я выберу "Одесса" из "user_state_id"
    И я в поле "user_phone" запишу "92834792834"
    И я нажму "Зарегистрироваться"


    И я увижу "Чтобы активировать ваш аккаунт, запишите в поле ниже присланный вам код подтверждения мобильного"
    И я в поле "phone_confirm" запишу "Не правельный код"
    И я нажму "Подтвердить"
    И я увижу "Не правельный код подтверждения мобильного телефона"
    И я перейду по ссылке "Повторно выслать код подтверждения"
    И я увижу "Код подтверждения выслан на мобильный с номером 92834792834"
    И я в поле "phone_confirm" запишу код для подтверждения мобильного "user@ya.ru"
    И я нажму "Подтвердить"
    ...
    И я перейду по ссылке "Отправить письмо повторно"
    И я увижу "Вам было повторно отправленно пиьмо с подтверждением элкетронной почты"
    И я нажму "Сохранить"

    Допустим я открою почту "user@ya.ru"
    И покажи все письма в "user@ya.ru"
    И на "user@ya.ru" должно было прийти "1" письмо с темой "Инструкции для подтверждения"
